...
function AccordionTrigger(props: React.ComponentProps<typeof AccordionPrimitive.Trigger>) {
  ...
  return (
    <AccordionPrimitive.Header className="flex w-full">
      ...
      >
        {children}
        {indicator === 'plus' && <Plus className="size-5 text-accent-1-900 shrink-0 transition-transform duration-200" strokeWidth={1} />}
        {indicator === 'arrow' && (
          <ChevronDown className="size-5 shrink-0 transition-transform duration-200" strokeWidth={1} />
        )}
      </AccordionPrimitive.Trigger>
