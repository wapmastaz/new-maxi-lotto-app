...
function DataGridColumnHeader<TData, TValue>({
  ...
  const headerLabel = () => {
    return (
      <div
        className={cn(
          'text-background uppercase font-medium inline-flex h-full items-center gap-1.5 text-xs leading-[calc(1.125/0.8125)] [&_svg]:size-3.5 [&_svg]:opacity-60',
          className,
        )}
      >
        {icon && icon}
  ...
  const headerButton = () => {
    return (
      <Button
        variant="ghost"
        className={cn(
          'text-background rounded-md font-medium uppercase -ms-2 px-2 h-7 hover:bg-secondary data-[state=open]:bg-secondary hover:text-foreground data-[state=open]:text-foreground',
          className,
        )}
