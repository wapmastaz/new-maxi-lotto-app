...
import { fetchUserPayouts } from '@/services/Transactions';
...
export const PayoutsHistory = () => {
  ...
  const { data: payouts, isFetching } = useQuery({
    queryKey: ['payouts', pagination],
    queryFn: () => fetchUserPayouts(pagination),
  ...
  const table = useReactTable({
    columns,
    data: payouts?.data ?? defaultData,
    rowCount: payouts?.totalCount,
    state: {
      pagination,
      sorting,
      // columnOrder,
  ...
  return (
    <DataGrid
      table={table}
      recordCount={payouts?.totalCount || 0}
      tableLayout={{
